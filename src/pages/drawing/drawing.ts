...
import { NavController, NavParams,ViewController,Platform,Content,LoadingController} from 'ionic-angular';
...
export class DrawingPage {
  ...
  constructor(public view:ViewController,public loading:LoadingController,public plt : Platform, public navParams: NavParams,public navCtrl: NavController) {
...
ctx3.drawImage(this.canvasElement, 0, 0);
  ...
  uploadImage(imageURI,index){
    ...
    console.log(imageURI);
    console.log("donE!!!!!!!!!!")
        var a = this.mypicref.child("images").child(this.receipt_number).child("signdata");
    this.encodeImageUri(imageURI, (image64)=>{
      a.putString(image64, 'data_url')
      .then(snapshot => {
        this.mypicref.child("images").child(this.receipt_number).child("signdata").getDownloadURL().then((url)=>{
